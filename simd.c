...
int main(void) {
    // On Apple Silicon the scheduler classifies a process by recent
    // behavior; a short CPU burst after I/O-bound activity (e.g. the
    // sync step in the sweep script) is often parked on E-cores under
    // QoS "utility", giving ~50-80% of P-core throughput. We're a
    // benchmark -- pin to user-initiated AND burn ~300ms of CPU so the
    // scheduler has unambiguous signal to promote to P-cores before
    // we start the actual timing loops.
#if defined(__APPLE__)
    pthread_set_qos_class_self_np(QOS_CLASS_USER_INITIATED, 0);
    {
        volatile double w = 0;
        double t = now_seconds();
        while (now_seconds() - t < 0.30) {
            for (int i = 0; i < 100000; i++) w += (double)i * 1.0001;
        }
    }
#endif
    simd_init();
